...
from music21 import converter, instrument, interval, stream
...
class ZoomableGraphicsView(QGraphicsView):
    """QGraphicsView with Ctrl+scroll zoom plus programmatic zoom helpers."""
    ...
    def zoom_in(self) -> None:
        self._apply_factor(self.STEP)

    def zoom_out(self) -> None:
        self._apply_factor(1 / self.STEP)
    ...
    def wheelEvent(self, event) -> None:
        if event.modifiers() & Qt.ControlModifier:
            self._apply_factor(self.STEP if event.angleDelta().y() > 0 else 1 / self.STEP)
            event.accept()
            return
...
class OctaveEditorDialog(QDialog):
    """Visual note editor for octave and free-pitch edits on a staff view."""

    # Pixels per semitone — shared by layout and drag-to-pitch math.
    PIX_PER_SEMITONE = 5.0

    class StaffNoteItem(QGraphicsEllipseItem):
        """Clickable, draggable notehead tied to a music21 note or chord event."""
        ...
        def __init__(
            ...
            self.dialog = dialog
            self._drag_start_scene_y: Optional[float] = None
        ...
        def itemChange(self, change, value):
            # Lock X during drag — pitch only changes with Y.
            if change == QGraphicsItem.ItemPositionChange:
                return QPointF(0.0, value.y())
            return super().itemChange(change, value)

        def mousePressEvent(self, event) -> None:
            if event.button() == Qt.LeftButton:
                self._drag_start_scene_y = self.scenePos().y()
            # Let Qt handle selection (respects Ctrl/Shift modifiers) and drag start.
            super().mousePressEvent(event)

        def mouseReleaseEvent(self, event) -> None:
            super().mouseReleaseEvent(event)
            if event.button() != Qt.LeftButton or self._drag_start_scene_y is None:
                return
            delta_y = self.scenePos().y() - self._drag_start_scene_y
            self._drag_start_scene_y = None
            if abs(delta_y) < OctaveEditorDialog.PIX_PER_SEMITONE / 2:
                return
            semitones = int(round(-delta_y / OctaveEditorDialog.PIX_PER_SEMITONE))
            if semitones == 0:
                return
            self.dialog._drag_shift_selection(self, semitones)
    ...
    def _init_ui(self) -> None:
        ...
        hint = QLabel(
            "Choose a part, optionally narrow the measure range, select notes, then click +8va. "
            "Ctrl+click to add/remove individual notes. Drag a note up or down to shift its pitch. "
            "Accidentals: \u266D / \u266E / \u266F buttons or Shift+- / Shift+0 / Shift++ . "
            "Zoom with Zoom +/- buttons, Ctrl++ / Ctrl+- / Ctrl+0, or Ctrl+scroll."
        )
        ...
        zoom_out_btn.setStyleSheet("background-color: #795548; color: white; padding: 8px 12px;")
        zoom_out_btn.clicked.connect(self.view.zoom_out)
        btn_row.addWidget(zoom_out_btn)

        zoom_reset_btn = QPushButton("1:1")
        zoom_reset_btn.setStyleSheet("background-color: #795548; color: white; padding: 8px 12px;")
        zoom_reset_btn.clicked.connect(self.view.zoom_reset)
        btn_row.addWidget(zoom_reset_btn)

        zoom_in_btn = QPushButton("Zoom +")
        zoom_in_btn.setStyleSheet("background-color: #795548; color: white; padding: 8px 12px;")
        ...
        QShortcut(QKeySequence("Ctrl++"), self, activated=self.view.zoom_in)
        QShortcut(QKeySequence("Ctrl+="), self, activated=self.view.zoom_in)
        QShortcut(QKeySequence("Ctrl+-"), self, activated=self.view.zoom_out)
        QShortcut(QKeySequence("Ctrl+0"), self, activated=self.view.zoom_reset)
        ...
        acc_row = QHBoxLayout()
        acc_label = QLabel("Accidentals:")
        acc_label.setStyleSheet("color: #444; font-size: 12px; padding-right: 4px;")
        acc_row.addWidget(acc_label)

        flat_btn = QPushButton("\u266D  Lower")
        flat_btn.setStyleSheet("background-color: #37474F; color: white; padding: 8px 16px;")
        flat_btn.clicked.connect(lambda: self._adjust_accidental_selected(-1))
        acc_row.addWidget(flat_btn)

        natural_btn = QPushButton("\u266E  Natural")
        natural_btn.setStyleSheet("background-color: #37474F; color: white; padding: 8px 16px;")
        natural_btn.clicked.connect(self._natural_selected)
        acc_row.addWidget(natural_btn)

        sharp_btn = QPushButton("\u266F  Raise")
        sharp_btn.setStyleSheet("background-color: #37474F; color: white; padding: 8px 16px;")
        sharp_btn.clicked.connect(lambda: self._adjust_accidental_selected(1))
        acc_row.addWidget(sharp_btn)

        acc_row.addStretch()
        layout.addLayout(acc_row)

        # Shift++ / Shift+= raise, Shift+- lower, Shift+0 natural.
        QShortcut(QKeySequence("Shift++"), self, activated=lambda: self._adjust_accidental_selected(1))
        QShortcut(QKeySequence("Shift+="), self, activated=lambda: self._adjust_accidental_selected(1))
        QShortcut(QKeySequence("Shift+-"), self, activated=lambda: self._adjust_accidental_selected(-1))
        QShortcut(QKeySequence("Shift+0"), self, activated=self._natural_selected)
    ...
    def _refresh_note_list(self) -> None:
        # Drop Python refs before scene.clear() destroys the C++ items.
        # Otherwise scene.clear() fires selectionChanged, whose handler
        # touches .isSelected() on already-deleted items and segfaults.
        self.note_items = []
    ...
    def _draw_measure(
        ...
        staff_top = origin_y + y_margin + 36
        staff_spacing = 18
        staff_left = origin_x + x_margin
        staff_right = origin_x + measure_width - x_margin
        note_area_width = staff_right - staff_left - 18
        ...
        for el in measure.notes:
            ...
            x = staff_left + 12 + (float(el.offset) / bar_ql) * note_area_width
            ...
            for p_idx, p in enumerate(sorted(pitches, key=lambda pitch: pitch.midi, reverse=True)):
                y = center_y - (p.midi - center_midi) * self.PIX_PER_SEMITONE + p_idx * 4
                ...
                stem = QGraphicsLineItem(x + 18, y + 7, x + 18, y - 38)
    ...
    def _transpose_events(self, items: List["OctaveEditorDialog.StaffNoteItem"], semitones: int) -> int:
        # Dedupe by event identity — chord notes share the same event_obj.
        # Octaves use a perfect-octave interval so spelling is preserved;
        # other shifts use the chromatic semitone count (music21 picks spelling).
        if semitones % 12 == 0 and semitones != 0:
            octaves = semitones // 12
            iv = interval.Interval(f"P{'' if octaves > 0 else '-'}{abs(octaves) * 7 + 1}")
        else:
    ...
    def _adjust_accidental_selected(self, delta: int) -> None:
        items = self._selected_note_items()
        if not items:
            QMessageBox.information(self, "No Selection", "Select one or more notes first.")
            return
        ...
        for item in items:
            ...
            if key in seen_events:
                continue
            seen_events.add(key)
            pitches = self._iter_event_pitches(el)
            if not pitches:
                continue
            touched = False
            for p in pitches:
                current = p.accidental.alter if p.accidental else 0
                new_alter = max(-2, min(2, int(current + delta)))
                if new_alter == current:
                    continue
                if new_alter == 0:
                    p.accidental = None
                else:
                    p.accidental = pitch_mod.Accidental(new_alter)
                touched = True
            if touched:
                changed += 1
        label = "raised" if delta > 0 else "lowered"
        self._log(f"Accidentals {label} on {changed} event(s)")
        if changed:
            self._dirty = True
        self._refresh_note_list()

    def _natural_selected(self) -> None:
        items = self._selected_note_items()
        if not items:
            QMessageBox.information(self, "No Selection", "Select one or more notes first.")
            return
        ...
        for item in items:
            ...
            if touched:
                changed += 1
        self._log(f"Naturals applied to {changed} event(s)")
        if changed:
            self._dirty = True
        self._refresh_note_list()

    def _drag_shift_selection(self, origin_item: "OctaveEditorDialog.StaffNoteItem", semitones: int) -> None:
        # If the dragged note is part of a multi-selection, apply the shift to
        # the whole selection; otherwise shift just the dragged event.
        selected = self._selected_note_items()
        if origin_item in selected and len(selected) > 1:
            items = selected
            scope = f"{len(set(id(i.event_obj) for i in items))} event(s)"
        else:
            items = [origin_item]
            scope = "1 event"
        changed = self._transpose_events(items, semitones)
        sign = "+" if semitones > 0 else ""
        self._log(f"Drag: {sign}{semitones} semitone(s) applied to {changed} of {scope}")
        self._dirty = True
        self._refresh_note_list()
    ...
    def _update_selection_summary(self) -> None:
        try:
            ...
            # A Qt item was deleted underneath us (e.g. during scene.clear()).
            # Skip this stale signal safely.
            return
        if not selected:
            self.selection_label.setText("No notes selected")
            return
    ...
    def _save(self) -> None:
        if not self.score:
            return
        target = Path(self.musicxml_path)
        tmp = target.with_suffix(target.suffix + ".tmp")
        try:
            self.score.write("musicxml", fp=str(tmp))
    ...
    def closeEvent(self, event) -> None:
        if self._dirty:
            reply = QMessageBox.question(
                self,
                "Unsaved Changes",
                "You have unsaved octave changes. Save before closing?",
