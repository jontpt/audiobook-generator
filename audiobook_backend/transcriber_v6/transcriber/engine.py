...
class ArrangementEngine:
    ...
    def _build_targets(
        ...
        for i, tname in enumerate(target_instruments):
            ...
            # Tessitura is a comfort preference, not permission to exceed the
            # instrument's absolute playable range.
            tess_lo = max(lo, tess_lo)
            tess_hi = min(hi, tess_hi)
            ext_lo = max(lo, ext_lo)
            ext_hi = min(hi, ext_hi)
    ...
    def _smooth_octave_jumps(
        ...
        for part in result_score.parts:
            ...
            for i in range(len(notes)):
                ...
                if best != curr:
                    # Transpose by a perfect octave so enharmonic spelling
                    # is preserved (setting .midi respells Gb as F#, etc.)
                    iv = interval.Interval("P8" if best > curr else "P-8")
                    notes[i].pitch.transpose(iv, inPlace=True)
                    midis[i] = best
                    corrections += 1
    ...
    def _shift_into_range(
        ...
        Tessitura hints are advisory only. Notes already inside the absolute
        playable range are left where they are instead of being re-octaved just
        to sit closer to a comfort band.
        """
        ...
        def _apply_octaves(target, delta: int) -> None:
            # Transpose by whole octaves, preserving enharmonic spelling.
            if delta == 0:
                return
            steps = delta // 12
            if steps == 0:
                return
            iv = interval.Interval(f"P{'' if steps > 0 else '-'}{abs(steps) * 7 + 1}")
            try:
                target.transpose(iv, inPlace=True)
            except Exception:
                # Fallback: direct midi adjustment if transpose fails
        ...
        if isinstance(element, note.Note):
            best = _best_for_context(element.pitch.midi)
            _apply_octaves(element, best - element.pitch.midi)
            ...
            if not chord_in_range and last_midi is not None:
                ...
                if rep - 12 >= hard_floor and abs(rep - 12 - last_midi) < abs(rep - last_midi):
                    shift -= 12
                elif max(mids) + shift + 12 <= hi and abs(rep + 12 - last_midi) < abs(rep - last_midi):
                    shift += 12
    ...
    def _shift_element_by_octaves(element, n_octaves: int) -> None:
        ...
        iv = interval.Interval(iv_name)
        try:
            element.transpose(iv, inPlace=True)
        except Exception:
