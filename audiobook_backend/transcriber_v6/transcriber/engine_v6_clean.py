"""
Arrangement engine for the Brass Ensemble Transcriber.

Core logic: register-aware source assignment, part rendering,
transposition, voicing rules, and gap filling.

Improvements over v5.9:
  - Type hints throughout
  - Removed _minimize_leap() no-op stub
  - Clearer section organization
  - Better docstrings
  - Vertical reduction restored with auto-activation heuristic
"""
...
class ArrangementEngine:
    ...
    def arrange(
        ...
        # Pre-processing: extract cross-voice melodic thread for dense scores
        if len(pitched_sources) >= len(target_instruments):
            from .melody_extractor import MelodicThreadExtractor
            melody_source = MelodicThreadExtractor(log_fn=self.log).extract(score, pitched_sources)
            if melody_source:
                self.log("  → Injecting melodic thread as primary melody source")
                pitched_sources.insert(0, melody_source)
    ...
    def _assign_parts_by_register(self, sources: List[SourceInfo], targets: List[TargetInfo]) -> None:
        ...
        if non_kb_targets:
            ...
            # Melodic thread always claims the top (highest-median) target first
            melody_src = next((s for s in sources if s.get("is_melody_thread")), None)
            if melody_src and non_kb_targets:
                top_tgt = non_kb_targets[0]
                top_tgt["assigned_sources"].append(melody_src)
                top_tgt["primary_source"] = melody_src
                assigned_indices.add(melody_src["index"])
                self.log(f"  {_safe(melody_src['name'])} -> {_safe(top_tgt['label'])} (melody thread, top slot)")

            remaining_sources = [s for s in sources if s["index"] not in assigned_indices]
            remaining_targets = [t for t in non_kb_targets if not t["primary_source"]]
            n_remaining = len(remaining_targets)

            activity_sorted = sorted(remaining_sources, key=lambda s: -s["_note_count"])
    ...
    def _render_single_part(
        ...
        for m_num in measure_nums:
            ...
            if source_m:
                for el in self._iter_measure_notes(source_m):
                    ...
                    if isinstance(el_copy, (note.Note, chord.Chord)):
                        ...
                        if not is_perc:
                            self._shift_into_range(el_copy, target_lo, target_hi, tess_lo, tess_hi, allow_pedal)
            ...
            if all_source_m_maps and not placed_spans and not sparse:
                best_src_m = None
                ...
                for src, m_map in all_source_m_maps:
                    ...
                    if not src_m:
                        continue
                    src_notes = [el for el in self._iter_measure_notes(src_m) if isinstance(el, (note.Note, chord.Chord))]
                    if not src_notes:
                        continue
                    src_med = sum(self._representative_midi(el, target) for el in src_notes) / len(src_notes)
                    reg_dist = abs(src_med - tgt_median)
            ...
            # Gap filling
            if all_source_m_maps and placed_spans:
                gap_candidates: Dict[float, Tuple[Any, float, float]] = {}
                for src, m_map in all_source_m_maps:
                    src_m = m_map.get(m_num)
                    if not src_m:
                        continue
                    for el in self._iter_measure_notes(src_m):
                        ...
                        dist = abs(midi - tgt_median)
                        if start not in gap_candidates or dist < gap_candidates[start][1]:
                            gap_candidates[start] = (copy.deepcopy(el), dist, ql)

                for start, (el_copy, _, clipped_ql) in sorted(gap_candidates.items()):
    ...
    def _shift_into_range(
        ...
        """Octave-shift a Note or Chord only if outside [lo, hi]."""
        ...
        def _apply_octaves(target, delta: int) -> None:
            if delta == 0:
                return
            steps = delta // 12
            if steps == 0:
                return
            iv_name = f"P{'' if steps > 0 else '-'}{abs(steps) * 7 + 1}"
            try:
                target.transpose(interval.Interval(iv_name), inPlace=True)
            except Exception:
                if isinstance(target, note.Note):
                    target.pitch.midi += delta
                elif isinstance(target, chord.Chord):
                    for p in target.pitches:
                        p.midi += delta

        if isinstance(element, note.Note):
            curr = element.pitch.midi
            _apply_octaves(element, _clamp(curr) - curr)
            ...
            shift = 0
            while min(mids) + shift < hard_floor:
                shift += 12
            while max(mids) + shift > hi and (min(mids) + shift - 12) >= hard_floor:
                shift -= 12
            _apply_octaves(element, shift)
    ...
    def _shift_element_by_octaves(element, n_octaves: int) -> None:
        """Shift all pitches in a Note or Chord by n_octaves, preserving spelling."""
        if n_octaves == 0:
            return
        iv_name = f"P{'' if n_octaves > 0 else '-'}{abs(n_octaves) * 7 + 1}"
        try:
            element.transpose(interval.Interval(iv_name), inPlace=True)
        except Exception:
