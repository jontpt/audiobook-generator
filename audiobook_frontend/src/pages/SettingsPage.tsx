    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <section className="bg-dark-800/60 border border-dark-700 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Button variant="outline" size="sm" icon={<Plus size={14} />} onClick={() => setAddKeyModal(true)}>
            Add Key
          </Button>
        </div>
        ...
        {keysLoading ? (
          <div className="space-y-2">{[1,2].map(i => <div key={i} className="h-14 bg-dark-700 rounded-xl animate-pulse" />)}</div>
        ) : apiKeys.length === 0 ? (
      ...
      <section className="bg-dark-800/60 border border-dark-700 rounded-2xl p-6">
        ...
        <form onSubmit={handlePasswordChange} className="space-y-4 max-w-sm">
          <Input label="Current password" type={showPwds ? 'text' : 'password'}
            value={pwdForm.current} onChange={e => setPwdForm(f => ({ ...f, current: e.target.value }))}
            icon={<Lock size={14} />} />
          <Input label="New password" type={showPwds ? 'text' : 'password'}
            value={pwdForm.next} onChange={e => setPwdForm(f => ({ ...f, next: e.target.value }))}
            hint="Minimum 8 characters" icon={<Lock size={14} />} />
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" className="text-xs text-dark-400 hover:text-white flex items-center gap-1"
      ...
      <Modal open={addKeyModal} onClose={() => setAddKeyModal(false)} title="Add API Key">
        <div className="space-y-4">
          <div>
            ...
            <select value={newKey.service} onChange={e => setNewKey(k => ({ ...k, service: e.target.value }))}
              className="w-full bg-dark-800 border border-dark-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/50">
              {Object.entries(SERVICE_META).map(([svc, meta]) => (
                <option key={svc} value={svc}>{meta.icon} {meta.name}</option>
              ))}
            </select>
          </div>
          <Input label="Label (optional)" placeholder={`My ${SERVICE_META[newKey.service]?.name} key`}
            value={newKey.label} onChange={e => setNewKey(k => ({ ...k, label: e.target.value }))} />
          <div>
            <Input label="API Key" type={showKey ? 'text' : 'password'}
              placeholder={SERVICE_META[newKey.service]?.hint}
              value={newKey.key} onChange={e => setNewKey(k => ({ ...k, key: e.target.value }))}
              icon={<Key size={14} />}
              iconRight={
                <button type="button" onClick={() => setShowKey(!showKey)} className="hover:text-white transition-colors">
                  {showKey ? <EyeOff size={14} /> : <Eye size={14} />}
                </button>
              } />
            <p className="text-xs text-dark-500 mt-1">🔒 Keys are encrypted and stored securely</p>
          </div>
          <div className="flex gap-3 pt-2">
            <Button variant="secondary" className="flex-1" onClick={() => setAddKeyModal(false)}>Cancel</Button>
            <Button variant="primary" className="flex-1"
              disabled={!newKey.key} loading={addKeyMutation.isPending}
