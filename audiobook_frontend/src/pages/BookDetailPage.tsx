...
import {
  ...
  Download, RefreshCw, Users, FileText, Clock, Layers, Wifi, WifiOff,
} from 'lucide-react';
...
// ── Processing pipeline steps visualiser ──────────────────────────────────────
...
export const BookDetailPage: React.FC = () => {
  ...
  // Live WebSocket progress (replaces polling)
  const ws = useBookProgress(id, book?.status ?? 'pending');

  // Merge: WS status takes priority over REST when in-flight
  const status   = ws.status   || book?.status   || 'pending';
  const progress = ws.progress ?? (book?.progress ?? 0);
  const message  = ws.message  || STEP_LABELS[status] || '';

  // When WS signals completion → refetch book from REST to get full data
  React.useEffect(() => {
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          {isProcessing && (
            <div className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full ${ws.connected ? 'bg-green-500/10 text-green-400' : 'bg-dark-700 text-dark-400'}`}>
              {ws.connected ? <Wifi size={12} /> : <WifiOff size={12} />}
              {ws.connected ? 'Live' : 'Polling'}
            </div>
          )}
